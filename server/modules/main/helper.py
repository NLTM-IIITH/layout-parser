...
def process_ganga_layout(folder_path: str) -> LayoutImageResponse:
    """
    Given a path to the folder if images, this function returns a list
    of word level bounding boxes of all the images
    """
    # call(f'/home/layout/layout-parser/ganga_layout.sh {folder_path}')
    os.system('docker run --rm --net host -v {}:/data layout:ganga-v0 python infer.py'.format(
        folder_path
    ))
    out = join(folder_path, 'out.json')
    with open(out, 'r') as f:
        r = json.loads(f.read())
    regions = []
    for i in r:
        regions.append(Region.from_xyxy(**i))
