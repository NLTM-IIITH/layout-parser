...
class Cage:
    ...
    def __init__(self, path_json, n_lfs):
        ...
        self.theta = torch.ones(
            (self.n_classes, self.n_lfs), device=self.device
        ).double()
    ...
    def fit_and_predict_proba(
        ...
        assert (
            (type(qt) == float and (qt >= 0 and qt <= 1))
            or (type(qt) == np.ndarray and (np.all(np.logical_and(qt >= 0, qt <= 1))))
            or (type(qt) == int and (qt == 0 or qt == 1))
        )

        assert (
            (type(qc) == float and (qc >= 0 and qc <= 1))
            or (type(qc) == np.ndarray and (np.all(np.logical_and(qc >= 0, qc <= 1))))
            or (type(qc) == int and (qc == 0 or qc == 1))
        )

        for temp in metric_avg:
            assert (
                temp in ["micro", "macro", "samples", "weighted", "binary"]
                or metric_avg is None
            )
        ...
        if self.n == None:
            self.n = torch.tensor(
                data[7], device=self.device
            ).double()  # Mask for s/continuous_mask
        else:
            assert torch.all(
                torch.tensor(data[7], device=self.device).double().eq(self.n)
            )
        if self.k == None:
            self.k = torch.tensor(data[8], device=self.device).long()  # LF's classes
        else:
            assert torch.all(
                torch.tensor(data[8], device=self.device).long().eq(self.k)
            )
        ...
        qt_ = (
            torch.tensor(qt, device=self.device).double()
            if type(qt) == np.ndarray
            else (torch.ones(self.n_lfs, device=self.device).double() * qt)
        )
        qc_ = (
            torch.tensor(qc, device=self.device).double()
        ...
        if path_test != None and path_log != None:
            ...
            file.write(
                "CAGE log:\tn_classes: {}\tn_LFs: {}\tn_epochs: {}\tlr: {}\n".format(
                    self.n_classes, self.n_lfs, n_epochs, lr
                )
            )
        elif path_test != None:
            print(
                "CAGE log:\tn_classes: {}\tn_LFs: {}\tn_epochs: {}\tlr: {}".format(
                    self.n_classes, self.n_lfs, n_epochs, lr
                )
            )
        ...
        with tqdm(total=n_epochs_) as pbar:
            for epoch in range(n_epochs_):
                ...
                prec_loss = precision_loss(
                    self.theta, self.k, self.n_classes, qt_, self.device
                )
                loss += prec_loss
                ...
                if path_test != None:
                    ...
                    if path_log != None:
                        file.write(
                            "Epoch: {}\ttest_accuracy_score: {}\n".format(
                                epoch, accuracy_score(y_true_test, y_pred)
                            )
                        )
                    else:
                        print(
                            "Epoch: {}\ttest_accuracy_score: {}".format(
                                epoch, accuracy_score(y_true_test, y_pred)
                            )
                        )
                    if epoch == n_epochs_ - 1:
                        print(
                            "final_test_accuracy_score: {}".format(
                    ...
                    for temp in metric_avg_:
                        if path_log != None:
                            file.write(
                                "Epoch: {}\ttest_average_metric: {}\ttest_f1_score: {}\n".format(
                                    epoch,
                                    temp,
                                    f1_score(y_true_test, y_pred, average=temp),
                                )
                            )
                        else:
                            print(
                                "Epoch: {}\ttest_average_metric: {}\ttest_f1_score: {}".format(
                                    epoch,
                                    temp,
                                    f1_score(y_true_test, y_pred, average=temp),
                                )
                        ...
                        if epoch == n_epochs_ - 1:
                            print(
                                "test_average_metric: {}\tfinal_test_f1_score: {}".format(
                                    temp, f1_score(y_true_test, y_pred, average=temp)
                                )
                            )
    ...
    def fit_and_predict(
        ...
        proba = self.fit_and_predict_proba(
            path_pkl, path_test, path_log, qt, qc, metric_avg, n_epochs, lr
        )
    ...
    def predict(self, path_test, qc=0.85, need_strings=False):
        ...
        return get_predictions(
            self.predict_proba(path_test, qc),
            self.class_map,
            self.class_dict,
            need_strings,
        )
